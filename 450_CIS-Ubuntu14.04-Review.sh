#!/bin/bash
echo "**********************************"
echo "***Ubuntu 14.04 – CIS Benchmark***"
echo "**********************************"
...
echo "   Command: 7.3.1 Disable IPv6 Router Advertisements (Not Scored)"
...
echo "   Command: if [ "`echo $PATH | grep :: `" != "" ]; then 
...
echo "   Command: for dir in `/bin/cat /etc/passwd | /bin/egrep -v '(root|halt|sync|shutdown)' | /usr/bin/awk -F: '($7 != "/usr/sbin/nologin") { print $6 }'`; do 
  if [ -d $dir ]; then 
...
echo "   Command: for dir in `/bin/cat /etc/passwd | /bin/egrep -v '(root|sync|halt|shutdown)' | /usr/bin/awk -F: '($7 != "/usr/sbin/nologin") { print $6 }'`; do 
...
echo "   Command: for i in $(cut -s -d: -f4 /etc/passwd | sort -u ); do 
...
echo "   Command: cat /etc/passwd | awk -F: '{ print $1 " " $3 " " $6 }' | while read user uid dir; do 
...
echo "   Command: /bin/cat /etc/passwd | /usr/bin/cut -f3 -d":" | /usr/bin/sort -n | /usr/bin/uniq -c | while read x ; do 
  [ -z "${x}" ] && break set - $x 
  if [ $1 -gt 1 ]; then 
...
echo "   Command: /bin/cat /etc/group | /usr/bin/cut -f3 -d":" | /usr/bin/sort -n | /usr/bin/uniq -c |  while read x ; do
    [ -z "${x}" ] && break 
 set - $x 
...
echo "   Command: cat /etc/passwd | /usr/bin/cut -f1 -d":" | /usr/bin/sort -n | /usr/bin/uniq -c | while read x ; do 
    [ -z "${x}" ] && break 
    set - $x 
 if [ $1 -gt 1 ]; then 
...
echo "   Command: cat /etc/group | /usr/bin/cut -f1 -d":" | /usr/bin/sort -n | /usr/bin/uniq -c | while read x ; do 
  [ -z "${x}" ] && break 
  set - $x 
  if [ $1 -gt 1 ]; then 
...
echo "   Command: for dir in `/bin/cat /etc/passwd |  /usr/bin/awk -F: '{ print $6 }'`; do 
